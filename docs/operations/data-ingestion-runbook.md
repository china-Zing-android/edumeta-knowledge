...
流水线：持久化原始 MD，解析 catalog/fact/source，提取 Markdown link/autolink/bare URL，PG staging，继承未变化 URL 的 WeKnora 状态，全局 OpenSearch 发布与数量校验，切换 current version，异步消费 WeKnora job。

相同 MD、parser contract 和院校元数据返回 `unchanged`，不发布索引也不新增 job。单 canonical URL 变化只新增一个 WeKnora job。版本切换时未完成 URL 会在新 current 版本续接轮询。解析或索引失败保留旧 current version。
